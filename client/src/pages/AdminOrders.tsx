              ...
              return (
                <div key={t} className="border border-gray-800 rounded-xl overflow-hidden">
                  <button
                    ...
                    className="w-full flex items-center justify-between px-4 py-3 text-sm text-gray-300 hover:text-white transition"
                  >
                    {TAB_LABELS[t]}
                    <span className="text-gray-500">({sectionOrders.length})</span>
                  </button>
                  {open && (
                          ...
                          {sectionOrders.map((order) => (
                            <div key={order.id} className="px-4 py-3 space-y-2">
                              <div className="flex items-center justify-between text-sm">
                                ...
                                <span className="text-gray-500">{order.user?.email || order.userId.slice(0, 8)}</span>
                              </div>
                              <div className="flex items-center justify-between text-sm">
                                <span className="font-medium">{order.product?.name || "—"}</span>
                                <span className="text-gray-400">${order.amountUsd}</span>
                              </div>
                ...
                {filtered.map((order) => (
                  <div key={order.id} className="border border-gray-800 rounded-xl p-4">
                    ...
                    {(order.status === "PENDING" || order.status === "CANCELLED") && (
                      <div className="border-t border-gray-800 mt-3 pt-3 flex items-center justify-end gap-2">
                        {order.status === "PENDING" && (
                          <>
                            <button
                              onClick={() => handleDeliver(order.id)}
